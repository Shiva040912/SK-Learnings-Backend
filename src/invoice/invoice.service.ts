...
export class InvoiceService {
  ...
  private buildMonthlyInstallmentSnapshot(student: StudentDocument) {
    if (
    ...
    return student.monthlyInstallments.map((installment) => ({
      installmentNumber: Number(installment.installmentNumber || 0),

      amount: this.roundMoney(Number(installment.amount || 0)),
  ...
  private buildPaymentHistorySnapshot(payments: PaymentDocument[]) {
  ...
  private getCurrentPayable(student: StudentDocument) {
    const pendingAmount = this.roundMoney(Number(student.pendingAmount || 0));
    ...
    if (student.feeType === 'monthly') {
      const currentInstallment = (student.monthlyInstallments || []).find(
        (installment) => installment.status !== 'paid',
      );

      if (currentInstallment) {
        return {
          amount: this.roundMoney(Number(currentInstallment.amount || 0)),
  ...
  private async getStudentPayments(studentId: Types.ObjectId) {
  ...
  private buildFeeSnapshot(
    ...
    const currentPayable = this.getCurrentPayable(student);
    ...
    return {
      ...
      feeEndingDate: student.feeDueDate || null,

      currentPayableAmount: currentPayable.amount,

      currentInstallmentNumber: currentPayable.installmentNumber || null,

      monthlyInstallments: this.buildMonthlyInstallmentSnapshot(student),

      paymentHistory: this.buildPaymentHistorySnapshot(payments),
    };
  }
  ...
  async createFeeSetupInvoice(studentId: string) {
    ...
    if (!student.feeSetupCompleted || !student.feeType) {
    ...
    const payments = await this.getStudentPayments(student._id);

    const feeSnapshot = this.buildFeeSnapshot(student, payments);
    ...
    const invoiceAmount =
      student.feeType === 'monthly'
        ? this.roundMoney(feeSnapshot.currentPayableAmount)
        : this.roundMoney(student.pendingAmount || student.totalFee);
    ...
    const invoice = new this.invoiceModel({
      ...
      dueDate: student.feeDueDate || null,
  ...
  async generateInvoicePdf(invoiceId: string) {
    ...
    return this.invoicePdfService.generatePdfBuffer(invoice);
  }

  async generateInvoicePdfByDocument(invoice: InvoiceDocument) {
    return this.invoicePdfService.generatePdfBuffer(invoice);
  }
}
