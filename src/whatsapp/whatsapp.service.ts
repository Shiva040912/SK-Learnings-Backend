...
type WhatsappComponent =
  WhatsappBodyComponent | WhatsappHeaderComponent | WhatsappButtonComponent;
...
export class WhatsappService {
  ...
  private normalizePhone(phone: string) {
    ...
    throw new BadRequestException('Enter a valid Indian WhatsApp number');
  }
  ...
  private formatFeeType(value: string) {
  ...
  private validateBodyParameters(parameters: string[]) {
    const hasEmptyValue = parameters.some(
      (value) =>
  ...
  async uploadPdf(pdfBuffer: Buffer, filename: string) {
    ...
    this.logger.log(`Uploading WhatsApp PDF: ${finalFilename}`);
    ...
    try {
      ...
      if (!response.ok || !result?.id) {
        this.logger.error(
          `WhatsApp PDF upload error: ${JSON.stringify(result, null, 2)}`,
        );
  ...
  async sendTemplate(data: {
    ...
    const bodyParameters = data.bodyParameters || [];

    this.validateBodyParameters(bodyParameters);
    ...
    if (data.buttonUrlParameter && data.buttonUrlParameter.trim()) {
      components.push({
        type: 'button',
        sub_type: 'url',
        index: '0',
        parameters: [
          {
            type: 'text',
            text: data.buttonUrlParameter.trim(),
          },
        ],
      });
    }
    ...
    this.logger.log(`Sending WhatsApp template: ${data.templateName}`);

    this.logger.log(`WhatsApp recipient: ${phone}`);

    this.logger.log(
      `WhatsApp body parameters: ${JSON.stringify(bodyParameters)}`,
    );

    if (data.mediaId) {
      this.logger.log(`WhatsApp document media ID: ${data.mediaId}`);
    }

    if (data.buttonUrlParameter) {
      this.logger.log(
    ...
    try {
      ...
      if (!response.ok) {
        this.logger.error(
          `WhatsApp API error: ${JSON.stringify(result, null, 2)}`,
        );
  ...
  async sendFeePaymentInvoice(data: {
    phone: string;
    parentName: string;
    studentName: string;
    studentId: string;
    ...
    feeEndingDate: Date | string;
    pdfBuffer: Buffer;
    invoiceNumber: string;
  }) {
    if (!data.parentName?.trim()) {
      throw new BadRequestException('Parent name is required for fee invoice');
    }

    if (!data.studentName?.trim()) {
      throw new BadRequestException('Student name is required for fee invoice');
    }
    ...
    const { mediaId, filename } = await this.uploadPdf(
      data.pdfBuffer,
    ...
    return this.sendTemplate({
      ...
      templateName: 'fee_payment_invoice',
       ...
       */
      buttonUrlParameter: data.studentId.trim(),
      ...
      bodyParameters: [
        ...
        this.formatAmount(data.totalFee),

        this.formatFeeType(data.feeType),

        this.formatAmount(data.pendingAmount),

        this.formatDate(data.feeEndingDate),
      ],
    });
  }

  async sendFeePaymentReminder(data: {
    phone: string;
    parentName: string;
    studentName: string;
    studentId: string;
    ...
    if (!data.parentName?.trim()) {
      throw new BadRequestException('Parent name is required for fee reminder');
    }
    ...
    return this.sendTemplate({
       ...
       */
      buttonUrlParameter: `${data.studentId.trim()}?source=reminder`,
  ...
  async sendFeePaymentReceipt(data: {
    phone: string;
