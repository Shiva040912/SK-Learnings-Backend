...
export class AcademicService {
  ...
  async createCourse(createCourseDto: CreateCourseDto) {
    const courseName = createCourseDto.courseName.trim();
  ...
  async getCourses() {
    return this.courseModel.find().sort({ courseName: 1 });
  }
  ...
  async createBatch(createBatchDto: CreateBatchDto) {
    const batchName = createBatchDto.batchName.trim();
    ...
    const batch = new this.batchModel({
      batchName,
      startTime: createBatchDto.startTime.trim(),
      endTime: createBatchDto.endTime.trim(),
    });
  ...
  async getBatches() {
    return this.batchModel.find().sort({ batchName: 1 });
  }
